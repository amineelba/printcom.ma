...
import { getNavCategories } from '@/lib/payload/cachedCategories'
...
export async function SiteHeader() {
  const [header, siteSettings, navCategories] = await Promise.all([
    getHeaderGlobal(),
    getSiteSettings(1),
    getNavCategories(),
  ])
      ...
      >
        <div className="mx-auto flex h-full max-w-(--pc-nav-max-width) items-center justify-between px-(--pc-page-gutter-small) md:px-(--pc-page-gutter-medium)">
          ...
          <div className="flex items-center gap-2">
            ...
            <MobileNavigation menus={menus} quoteCTA={quoteCTA} navCategories={navCategories} />
          </div>
        </div>
      </header>
